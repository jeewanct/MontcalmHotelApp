...
class OffersViewCell: UICollectionViewCell {
    ...
    var offers: OffersTabBarHotelOffers?{
        didSet{
            offerHeading.text = offers?.offerName

            if let imageUrl = offers?.offerImageUrl, let imageUrlRequest = URL(string: imageUrl){
    ...
    @objc func handleKnowMore(){
        ...
        if let window = UIApplication.shared.keyWindow{
            ...
            guard let knowMoreDetail = offers?.longDesc else{
                return
            }
